-- Batch jobs table (must be created before accounts due to FK reference)
...
CREATE INDEX IF NOT EXISTS idx_email_domains_active ON email_domains(is_active);
CREATE INDEX IF NOT EXISTS idx_email_domains_priority ON email_domains(priority);
...
CREATE INDEX IF NOT EXISTS idx_blacklisted_domains_domain ON blacklisted_domains(domain);
...
CREATE INDEX IF NOT EXISTS idx_registration_attempts_batch_job_id ON registration_attempts(batch_job_id);
...
-- Apply trigger to tables with updated_at (use IF NOT EXISTS pattern)
DO $$ BEGIN
